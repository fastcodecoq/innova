...
function Close_Button($classes) {
    ...
    echo $GLOBALS["ONESCRIPT"].'?i='.substr($_GET["f"],0,strrpos($_GET["f"],"/")).'\'" />';
}
...
if ($page == "index") {
    ...
    for ($x=1; $x < $levels; $x++) {
        if ($x !== 1){ $current_path .= '/'; }
        $current_path = $GLOBALS['current_path'].$path_levels[$x];
        echo '<a href="'.$ONESCRIPT.'?i='.$GLOBALS['current_path'].'" class="path"> ';
...
<!--============== List files - BLOCK view ===============-->
    ...
    function list_view() {
        ...
        foreach ($files as $file) {
            ...
            <tr>
                <td>
                    <?php echo "<a href='", $GLOBALS['ONESCRIPT'],"?f=", $file, "'"; ?>
